...
This command allows users of the TaskWise application to add optional remarks to people in their address book and edit it if required. The command should have the following format:
...
Now that we have our `RemarkCommand` ready to be executed, we need to update `TaskWiseParser#parseCommand()` to recognize the `remark` keyword. Add the new command to the `switch` block by creating a new `case` that returns a new instance of `RemarkCommand`.
...
:information_source: Don’t forget to update `TaskWiseParser` to use our new `RemarkCommandParser`!
...
TaskWise stores data by serializing `JsonAdaptedTask` into `json` with the help of an external library — Jackson. Let’s update `JsonAdaptedTask` to work with our new `Task`!

While the changes to code may be minimal, the test data will have to be updated as well.

<div markdown="span" class="alert alert-warning">

:exclamation: You must delete TaskWise’s storage file located at `/data/addressbook.json` before running it! Not doing so will cause TaskWise to default to an empty address book!
...
This concludes the tutorial for adding a new `Command` to TaskWise.
