...
1. _Step into_ the line where user input in parsed from a String to a Command, which should bring you to the `TaskWiseParser#parseCommand()` method (partial code given below):
   ``` java
...
1. Similar to before, you can step over/into statements in the `LogicManager#execute()` method to examine how the control is transferred to the `Storage` component and what happens inside that component.
   ...
   <div markdown="span" class="alert alert-primary">:bulb: **Intellij Tip:** When trying to step into a statement such as `storage.saveTaskWise(model.getTaskWise())` which contains multiple method calls, Intellij will let you choose (by clicking) which one you want to step into.
   </div>

1.  As you step through the code inside the `Storage` component, you will eventually arrive at the `JsonTaskWise#saveTaskWise()` method which calls the `JsonSerializableTaskWise` constructor, to create an object that can be _serialized_ (i.e., stored in storage medium) in JSON format. That constructor is given below (with added line breaks for easier readability):

    **`JsonSerializableTaskWise` constructor:**
    ``` java
    /**
     * Converts a given {@code ReadOnlyTaskWise} into this class for Jackson use.
     *
     * @param source future changes to this will not affect the created
     * {@code JsonSerializableTaskWise}.
     */
...
1. It appears that a `JsonAdaptedTask` is created for each `Task` and then added to the `JsonSerializableTaskWise`.
